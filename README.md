...
## Getting started
### Start From Scratch
> You can clone from this repo to start from scratch
...
```bash
$ yarn install
$ yarn start # By default, templates will be created in `./dist` and HTML & CSS files in './templates' will be watched for changes.
```
...
#### Using `inlineRemoteUrl` & `base64`
If you wish to use your own hosted images into templates, enable `inlineRemoteUrl` in your `conf.json`:
```json
  "inlineRemoteUrl" : true,
```

And also if you wish to disable convert images into `base64` format, set `base64` to `false`:
```json
  "base64": false
```
...
> NB: config variables **also accept HTML**. That's useful for including links. You also MUST use `<br/>` tag if you want to make a break line 😉
